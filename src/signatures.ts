...
export function verifySignatures(transcript: Record<string, JsonValue>): SignaturesResult {
  const errors: string[] = [];
  ...
  for (let i = 0; i < rounds.length; i++) {
    ...
    const sig = sigObj as Record<string, JsonValue>;
    ...
    try {
      pubBytes = bs58.decode(pubKeyB58);
    } catch {
