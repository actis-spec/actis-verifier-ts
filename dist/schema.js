...
function validateSchema(transcriptJson) {
    const errors = [];
    // Parse JSON.
    let transcript;
    try {
    ...
    for (let i = 0; i < rounds.length; i++) {
        ...
        for (const field of REQUIRED_ROUND_FIELDS) {
            if (!(field in r))
                errors.push(`Round ${i} missing required field: ${field}`);
        }
        ...
        if (typeof sig === "object" && sig !== null && !Array.isArray(sig)) {
            ...
            if (!("signer_public_key_b58" in s) || !("signature_b58" in s)) {
                errors.push(`Round ${i} signature missing signer_public_key_b58 or signature_b58`);
            }
        }
    }
